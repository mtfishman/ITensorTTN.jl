...
ttntags(; c = nothing, d = nothing, g = nothing) =
    reduce(addtags, [celltags(c), childtags(d), generationtags(g)])
...
function InfiniteTTN(s::Vector{IndexT}, st; z::Integer, linkdims) where {IndexT<:Index}
    N = length(s)
    s∞ = infsiteinds(s)
    product_state = [state(s∞[n], st(n)) for n = 1:N]
...
function ITensorInfiniteMPS.TransferMatrix(
    ...
    return ITensorMap(
        [A, Adag];
        input_inds = (dag(l_in), l_in'),
        output_inds = (l_out, dag(l_out)'),
    )
end
